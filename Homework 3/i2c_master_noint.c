...
void i2c_master_init(void) {
    unsigned char control_write_byte = 0b01000000; // create write address for chip
    unsigned char register_byte = 0x00; // chooses A-pins direction
    unsigned char value_byte = 0x00; // makes A_pins outputs
    i2c_master_start(); // send start bit
    i2c_master_send(control_write_byte);
    i2c_master_send(register_byte);
    i2c_master_send(value_byte);
    i2c_master_stop();
    register_byte = 0x01; // chooses B_pins direction
    value_byte = 0xFF; // makes B_pins inputs
    i2c_master_start();
    i2c_master_send(control_write_byte);
    i2c_master_send(register_byte);
    i2c_master_send(value_byte);
...
void set_pin_high(void) {
    unsigned char control_write_byte = 0b01000000; // create write address for chip
    unsigned char register_byte = 0x14;
    unsigned char value_byte = 0xFF;
    i2c_master_start(); // send start bit
    i2c_master_send(control_write_byte);
    i2c_master_send(register_byte);
    i2c_master_send(value_byte);
...
unsigned char get_pin_value(void) {
    unsigned char control_write_byte = 0b01000000;
    unsigned char register_byte = 0x13;
    unsigned char control_read_byte = 0b01000001;
    i2c_master_start(); // send start bit
    i2c_master_send(control_write_byte);
    i2c_master_send(register_byte);
    i2c_master_restart();
    i2c_master_send(control_read_byte);
    unsigned char return_value = i2c_master_recv();
    i2c_master_ack(1);
    i2c_master_stop();
    return return_value;
}
